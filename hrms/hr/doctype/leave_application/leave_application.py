...
class LeaveApplication(Document, PWANotificationsMixin):
	...
	def on_update(self):
		# Simple workflow like Material Request - no approval setup during creation
	...
	def on_submit(self):
		# Simple workflow like Material Request - just set status to "Pending Approval"
		if self.status == "Open":
			self.db_set("status", "Pending Approval")

		# Auto-populate approvers table using approval chain logic (like Material Request)
		# Only populate if not already populated
		if not self.approvers or len(self.approvers) == 0:
			try:
				self.setup_approvers_table()
				frappe.log_error(f"Auto-populated approvers table during submission for employee {self.employee}", "Approvers Auto-Setup")
			except Exception as e:
				frappe.log_error(f"Error setting up approvers table during submission: {str(e)}", "Approvers Setup Error")

		self.validate_back_dated_application()
		self.update_attendance()
		self.validate_for_self_approval()

		# notify leave applier about submission (not approval yet)
		if frappe.db.get_single_value("HR Settings", "send_leave_notification"):
		...
		if self.leave_approver:
			try:
				self.notify_leave_approver()
				frappe.log_error(f"Initial email notification sent to first approver: {self.leave_approver}", "Email Notification")
			except Exception as e:
				frappe.log_error(f"Error sending initial email to approver: {str(e)}", "Email Notification Error")

		# Only create leave ledger entry if approved
	...
	def setup_approvers_table(self):
		"""Setup the approvers table exactly like Material Request - auto-populated from approval chain"""
		try:
			# Get the approval chain using our existing logic
			approval_chain = get_leave_approval_chain(self.employee)

			if approval_chain:
				frappe.log_error(f"Setting up approvers table: {approval_chain}", "Approvers Setup")

				# Clear existing approvers and populate with approval chain
				self.approvers = []
				...
				# Set respect_approver_order to True for sequential approval
				self.respect_approver_order = 1

				# Set leave_approver to the first approver for email notifications
				if approval_chain:
					self.leave_approver = approval_chain[0]
					frappe.log_error(f"Set leave_approver to first approver: {self.leave_approver}", "Approvers Setup")

				frappe.log_error(f"Approvers table setup completed with {len(approval_chain)} approvers", "Approvers Setup")
			else:
				frappe.log_error(f"No approval chain found for employee {self.employee}", "Approvers Setup")
		except Exception as e:
			frappe.log_error(f"Error in setup_approvers_table: {str(e)}", "Approvers Setup Error")
...
def get_hr_manager():
	...
	hr_manager = frappe.db.sql("""
		SELECT DISTINCT u.name 
		FROM `tabUser` u
		JOIN `tabHas Role` r ON u.name = r.parent
		WHERE u.enabled = 1 AND r.role = 'HR Manager'
		LIMIT 1
	""", as_list=True)
...
def get_managing_director():
	...
	md = frappe.db.sql("""
		...
		FROM `tabUser` u
		JOIN `tabHas Role` r ON u.name = r.parent
		WHERE u.enabled = 1 AND r.role = 'Managing Director'
		LIMIT 1
	""", as_list=True)
...
def populate_approvers_from_chain(employee):
	"""Get approvers list for auto-populating the approvers table"""
	try:
		approval_chain = get_leave_approval_chain(employee)
		return {"approvers": approval_chain}
	except Exception as e:
		frappe.log_error(f"Error getting approval chain for employee {employee}: {str(e)}", "Approval Chain Error")
		return {"approvers": []}

@frappe.whitelist()
def fix_document_approval_chain(name):
	"""Fix approval chain for a specific document - manual utility"""
	try:
		frappe.log_error(f"Manual fix of approval chain for document {name}", "Approval Chain Fix")
		doc = frappe.get_doc("Leave Application", name)

		# Get approval chain
		approval_chain = get_leave_approval_chain(doc.employee)
		frappe.log_error(f"Retrieved approval chain for employee {doc.employee}: {approval_chain}", "Approval Chain Fix")

		if approval_chain:
			# Clear existing approval chain
			doc.approval_chain = []

			# Populate approval chain
			for i, approver in enumerate(approval_chain, 1):
				doc.append("approval_chain", {
					"approval_level": i,
					"approver": approver,
					"status": "Pending"
				})
			...
			doc.current_approver = approval_chain[0] if approval_chain else None
			doc.leave_approver = approval_chain[0] if approval_chain else None

			# Save the document
			doc.save()
			frappe.db.commit()

			frappe.log_error(f"Successfully fixed approval chain for {name} - count: {len(doc.approval_chain)}", "Approval Chain Fix")
			return f"Successfully fixed approval chain. Found {len(approval_chain)} approvers: {', '.join(approval_chain)}"
		else:
			return "No approval chain found for this employee"

	except Exception as e:
		frappe.log_error(f"Error fixing approval chain for {name}: {str(e)}", "Approval Chain Fix")
	...
	try:
		...
		for approver in doc.approvers:
			if approver.approver == frappe.session.user and approver.status == "New":
				...
				frappe.db.commit()
				frappe.msgprint(
					_("Leave application has been Approved" if str(action) == "1" else "Leave application has been Rejected"),
					alert=True,
		...
		# Update the status field and set next approver for email notifications
		...
		if approved == len(doc.approvers):
			...
			# Create leave ledger entry when fully approved
			...
			for approver in doc.approvers:
				if approver.status == "New":
					...
					break
			if next_approver:
				doc.db_set("leave_approver", next_approver)
				frappe.log_error(f"Set next approver for email notifications: {next_approver}", "Approval Chain Update")

				# Send email notification to next approver
				try:
					doc.notify_leave_approver()
					frappe.log_error(f"Email notification sent to next approver: {next_approver}", "Email Notification")
				except Exception as e:
					frappe.log_error(f"Error sending email to next approver: {str(e)}", "Email Notification Error")

			doc.save()
			frappe.db.commit()

	except Exception as e:
		frappe.log_error(f"Error in approve_reject: {str(e)}", "Leave Application Approval Error")
		frappe.msgprint(
			_("An error occurred during approval. Please try again."),
			alert=True,
			indicator="Red",
		)
